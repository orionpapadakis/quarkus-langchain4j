...
1. Ensure your `pom.xml` contains the `quarkus-langchain4j-gpu-llama3 dependency`.

2. Configure the TornadoVM argfile in your pom properties:

[source,xml]
----
<properties>
    <tornado.argfile>${env.TORNADOVM_SDK}/../../../tornado-argfile</tornado.argfile>
</properties>
----

3. Pass the argfile to the JVM in the `<configuration>` section:
...
Your endpoints (blocking and/or streaming) will now automatically leverage GPULlama3 on GPUs.
...
⚠ **Important:**️ Ensure the environment variables `TORNADOVM_SDK` and the `tornado-argfile` path are correctly set.
